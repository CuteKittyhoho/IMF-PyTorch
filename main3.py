...
if args.model in ['ConvE', 'TuckER', 'Mutan', 'IMF', 'IMFFFT', 'IMFWavelet']:
...
if args.image_features:
    #args.img = F.normalize(img_features, p=2, dim=1)
    args.img = F.normalize(torch.Tensor(img_features), p=2, dim=1)
if args.text_features:
    #args.desp = F.normalize(text_features, p=2, dim=1)
...
def train_decoder(args):
    ...
    for epoch in range(args.epochs):
        ...
        with tqdm(total=corpus.max_batch_num, desc=f'Epoch {epoch+1}/{args.epochs}', unit='batch') as pbar:
